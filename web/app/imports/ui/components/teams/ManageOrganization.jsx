...
export const ManageOrganization = (props) => {
  ...
  async function handleEmailChange(member, prop) {
    const result = await confirmService.show({
      ...
      actionItems: [
        { label: 'Send New Enrollment Email', return: true },
        { label: 'Cancel', return: false },
      ],
    })
    if (result) {
      ...
      if (newEmail === member.emails[0].address) {
        dispatch({
          ...
          globalMessage: {
            visible: true,
            isSnackBar: true,
            text: `No enrollment email sent: the email stayed the same (${newEmail}).`,
            severity: 'error',
            autoHide: 10000,
          },
        })
        ...
        if (others > 0) {
          dispatch({
            ...
            globalMessage: {
              visible: true,
              isSnackBar: true,
              text: `This email is already in use, please choose a different one or contact support`,
              severity: 'error',
              autoHide: 50000,
            },
          })
          ...
          const update = await promisify(
            Meteor.call,
            CONSTANTS.methods.users.update,
            { [prop]: newEmail, userId: member._id }
          )
          if (update) {
            dispatch({
              type: 'canvas',
    ...
    try {
      if (currentInvites.length > 0) {
        ...
        dispatch({
          ...
          globalMessage: {
            visible: true,
            isSnackBar: true,
            text: `You haven't added any team members to invite yet!`,
            severity: 'error',
            autoHide: 10000,
          },
        })
      ...
      dispatch({
        ...
        globalMessage: {
          visible: true,
          isSnackBar: true,
          text: `There was a problem inviting these users: ${err.message}`,
          severity: 'error',
          autoHide: 10000,
        },
      })
  ...
  async function resendEnrollment(user) {
    ...
    if (res) {
      await promisify(Meteor.call, CONSTANTS.methods.users.resendEnrollment, {
        users: [user],
      })
      dispatch({
        type: 'canvas',
        globalMessage: {
          ...
          isSnackBar: true,
          severity: 'info',
          text: `Enrollment email resent to ${user.emails[0].address}`,
          autoHide: 10000,
        },
      })
    }
    ...
    <>
      <TableContainer component={Paper} className={classes.root}>
        <Table className={classes.table} stickyHeader size="small">
          <TableHead>
            <TableRow>
              <TableCell className={classes.sticky}>Name / Email</TableCell>
              {claims.map((claim) => (
              ...
              <TableRow>
                <TableCell colSpan={6} align="center">
                  <Typography color="secondary">
                    You have no team members defined. Invite some below!
                  </Typography>
                </TableCell>
              </TableRow>
            )}
              ...
              >
                {AuthManager.userHasClaim(Meteor.userId(), [
                  ...
                  <TableCell className={classes.sticky}>
                    <Grid container alignItems="top" justify="flex-end">
                      <Grid item xs={2}>
                        {AuthManager.userHasClaim(Meteor.userId(), [
                          CONSTANTS.claims.canRemoveUsers._id,
                        ]) && (
                          <IconButton
                            aria-label="delete"
                            onClick={(e) => {
                      ...
                      <Grid item xs={2}>
                        {member &&
                          member.emails &&
                          member.emails[0] &&
                          !member.emails[0].verified && (
                            <Tooltip
                              color="secondary"
                              title="User has not enrolled in the team yet. Click to resend the enrollment email."
                            >
                              <IconButton
                      ...
                      <Grid item xs={4}>
                        <TextField
                          ...
                          fullWidth
                          value={member.profile ? member.profile.firstName : ''}
                          onChange={(e) =>
                            handleChange(member, 'firstName', e.target.value)
                          }
                          InputLabelProps={{
                            style: { color: '#fff' },
                          }}
                      ...
                      <Grid item xs={4}>
                        <TextField
                          ...
                          fullWidth
                          value={member.profile ? member.profile.lastName : ''}
                          onChange={(e) =>
                            handleChange(member, 'lastName', e.target.value)
                          }
                          InputLabelProps={{
                            style: { color: '#fff' },
                          }}
                ...
                {claims.map((claim) => (
                  <TableCell key={claim._id}>
                    <Tooltip
                      title={
                        isDisabled(member, claim) ? (
                          <>
                            You do not have permission to change{' '}
                            {claim.label.toLowerCase()}
                          </>
                        ) : (
                          <>{claim.label} </>
                        )
                    ...
                    >
                      <Checkbox
                        name={claim._id}
                        checked={isChecked(member, claim)}
                        onClick={(e) => {
                          handlePermission(member, claim, e.target.checked)
                        }}
                        disabled={isDisabled(member, claim)}
                      />
                    </Tooltip>
                  </TableCell>
                ))}
