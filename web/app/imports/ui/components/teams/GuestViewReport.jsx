...
export const GuestViewReport = (props) => {
  ...
  let unlistedViewsThisMonth = data?.totalUnlistedViewsByMonth && data?.totalUnlistedViewsByMonth[new Date().getMonth() + 1] ? data?.totalUnlistedViewsByMonth[new Date().getMonth() + 1] : 0;
  ...
  return (
    <Grid container justify="flex-end" alignItems="center" spacing={2}>
      ...
      <Grid item>
        <Typography variant="h1" color="secondary" style={{ letterSpacing: "-10px" }}>
          {unlistedViewsThisMonth} / {data?.allowableUnlistedViewsPerMonth}
        </Typography>
      </Grid>
      <Grid item style={{ marginLeft: "10px" }}>
        <Typography variant="overline" color="secondary">
          {data?.allowableUnlistedViewsPerMonth - unlistedViewsThisMonth} passes left in {new Date().toLocaleString('default', { month: 'long' })}
        </Typography>
      </Grid>
          ...
          <Table stickyHeader size="small">
            ...
            <TableBody>
              {data?.dataRows?.map((row, index) => (
                <TableRow key={index} className={index % 2 ? classes.lightRow : classes.darkRow}>
                  {row?.map((rx, index) => (
                    <TableCell key={index}>
                      {rx}
                    </TableCell>
      ...
      <Grid item style={{ marginLeft: "10px", color: "#fff" }}>
        <Typography variant="h5">
          No guest views yet in {new Date().toLocaleString('default', { month: 'long' })}
        </Typography>
      </Grid>
      }
