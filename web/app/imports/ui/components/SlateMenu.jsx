...
export const SlateMenu = (props) => {
  ...
  async function deleteSlate() {
    ...
    if (result) {
      try {
        await promisify(Meteor.call, CONSTANTS.methods.slates.remove, { slateId: props.slate._id });
        dispatch({ type: "displayslates", invokeRerender: Random.id() });
      } catch (err) {
        dispatch({ type: "canvas", globalMessage: { visible: true, text: `Error deleting: ${err.message}`, severity: "error", autoHide: 60000 } });
      }
    }
  }
  ...
  async function changeSlatePrivacy() {
    ...
    function enact() {
      ...
      Slates.update({ _id: props.slate._id }, { $set: { "options.isPublic": isPublic, "options.isPrivate": isPrivate, "options.isUnlisted": isUnlisted }});
      dispatch({ type: "displayslates", invokeRerender: Random.id() });
    }
    if ( ((!Meteor.user().orgId && !["solo_monthly", "solo_yearly"].includes(Meteor.user().planType) || Organizations.findOne()?.planType === "free")) && props.slate.options.isPublic) {
  ...
  return (<>
  {props.isTemplate ? 
    <Button variant="contained" color="secondary" style={{minWidth: "160px"}} onClick={((e) => { e.stopPropagation(); copy(); })}>
      Use Template
    </Button>
    :
    <>
      <IconButton aria-controls="slate-menu" aria-haspopup="true" color="inherit" onClick={((e) => { showSlateMenu(e); })}>
        <MoreVertIcon />
      </IconButton>
      <Menu
        id="slate-menu"
      ...
      >
        <MenuItem onClick={(e) => {
          ...
          {(!props.isCommunity && !props.isTemplate && (Meteor.userId() === props.slate.userId || AuthManager.userHasClaim(Meteor.userId(), [CONSTANTS.claims.admin._id]))) &&
          ...
          {(!props.isCommunity && !props.isTemplate && Meteor.userId() === props.slate.userId) &&
            <MenuItem onClick={( async (e) => { 
              hideSlateMenu(e);
              changeSlatePrivacy()
            })}>
              <ListItemIcon style={{ marginRight: "-25px" }}>
                {props.slate.options.isPublic ? <LockOpenIcon/> :  <LockIcon/> }
              </ListItemIcon>
              <Typography variant="inherit">
                {props.slate.options.isPublic ? "Make private" : "Make public"}
              </Typography>
            </MenuItem>
          }
      </Menu>
