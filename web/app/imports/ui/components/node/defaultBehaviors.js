function defaultBehaviors(node) {
  return {
    showDelete: {
      title: 'Show Delete',
      description:
        "Allow the delete icon to be shown, even if it's the last node.",
      disabled: node?.options?.showAddAndDeleteConditionally,
      default: true,
    ...
    showColorTab: {
      ...
      default: true,
      unsetForDisabled: true,
      disabled: node?.options?.disableMenuAsTemplate,
      category: 'menu',
      col: 4,
    },
    showTextTab: {
    ...
    spaceBetweenNodesWhenAdding: {
      ...
      disabled:
        !node?.options?.showAdd &&
        !node?.options?.showAddAndDeleteConditionally,
      default: 30,
      category: 'node',
      col: 12,
    },
    showAddAndDeleteConditionally: {
      ...
      description:
        'Only show the delete or add icon for the most recent copy of this node (You must uncheck both show delete and show add for this to be available).',
      disabled: node?.options?.showDelete || node?.options?.showAdd,
      default: false,
      unsetForDisabled: true,
      category: 'node',
      col: 12,
    ...
    disableMenuAsTemplate: {
      ...
      category: 'menu',
      disabled:
        node?.options?.showColorTab ||
        node?.options?.showTextTab ||
        node?.options?.showShapeTab ||
        node?.options?.showImageTab ||
        node?.options?.showEffectTab,
      col: 6,
    },
  }
}
