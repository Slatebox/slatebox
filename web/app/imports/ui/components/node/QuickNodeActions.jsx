...
export const QuickNodeActions = (props) => {
  ...
  let baseOpts = cloneDeep(props.slate?.nodes?.allNodes[props.slate?.nodes?.allNodes.length - 1]?.options) ||
    { backgroundColor: "#23aad6", foregroundColor: "#000", lineColor: "#333", lineWidth: 3 }
  ...
  const addNode = (shape) => {
    ...
    function setDimens(tString) {
      ...
      ["text", "groupId", "isComment", "id", "image", "xPos", "yPos", "height", "width", "vectorPath", "textBounds", "textOffset", "opacity", "borderOpacity", "textOpacity"].forEach(p => {
        delete bcopy[p];
      });
  ...
  const actionPaths = chunk(defaultShapes, 6).map((ds, i) => { return { row: i, shapes: ds } });
  let actionMenuStyle = actionMenuOpen ? { backgroundColor: "#fff", border: "1px solid #000", borderBottomRightRadius: "30px", borderTopLeftRadius: "10px" } : {};
        ...
        {row.shapes.map((shape) => (
          <Grid key={shape.key} item xs={2}>
            <Tooltip title={shape.name} placement="top" aria-label={shape.key}>
              <SvgIcon style={{ color: baseOpts.backgroundColor, "fontSize": 42 }} onClick={() => { addNode(shape); }}>
  ...
  return (
    <div>
      <SpeedDial
        ariaLabel={props.commentAccessOnly ? "Slatebox - comment" : "Slatebox - quickly add a node"}
        ...
        direction="up"
      >
        {!props.commentAccessOnly && actionPaths.map((action) => (
            <SpeedDialAction
              key={action.row}
              tooltipTitle=""
            ...
            <Grid container justify="center" alignItems="center">
              {customActions.map((c) => (
                <Grid key={c.key} item xs={4}>
                  <Button variant="outlined" color="secondary" style={{ height: "47px", width: "90%" }} onClick={() => { addNode(c); }}>
