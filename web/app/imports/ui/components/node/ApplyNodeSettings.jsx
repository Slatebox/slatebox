...
import node from '../../../api/client/slatebox/core/node';
...
export const ApplyNodeSettings = (props) => {
  ...
  const possibleStyles = [
    { style: "color", title: "Node Color", description: "Apply node color and opacity" },
    { style: "relationships", title: "Node Relationshps", description: "Apply line color, opacity, and effect" },
    { style: "border", title: "Node Border", description: "Apply border color, style, and opacity" },
    { style: "text", title: "Node Text", description: "Apply font, size, color, opacity, and effect" },
    ...
    { style: "behavior", title: "Node Behavior", description: "Apply node behaviors" }
  ];
  ...
  function invoke(opts) {
    allOtherNodeIds.forEach(nid => {
      opts.packages.forEach(p => {
        let c = true;
        if (opts.compare) {
          const node = props.node.slate.nodes.allNodes.find(n => n.options.id === nid);
          c = opts.compare(node);
        }
        if (c) {
  ...
  async function invokePathChanges() {
    ...
    const sendPath = Slatebox.utils._transformPath(options.vectorPath, `T${options.xPos * -1},${options.yPos * -1}`);

    // next calculate the specific paths for each node on the canvas
    const nodes = props?.node?.slate.nodes.allNodes.filter(n => getOtherNodes().includes(n.options.id));
    for (let node of nodes) {
      let vbbox = Slatebox.utils.getBBox({ path: sendPath });
      let path = await promisify(Meteor.call, CONSTANTS.methods.slates.scale, { path: sendPath, width: node.options.width/vbbox.width, height: node.options.height/vbbox.height });
      exe({ type: "onNodeShapeChanged", data: { id: node.options.id, shape: path, width: vbbox.width, height: vbbox.height } });
      ...
      switch (style) {
        ...
        case "color": {
          invoke({ 
            ...
            compare: ((node) => {
              return node.options.backgroundColor !== options.backgroundColor || node.options.opacity !== options.opacity; 
            })
          });
          break;
        }
        case "relationships": {
          ...
          const nodes = props?.node?.slate.nodes.allNodes.filter(n => allOtherNodeIds.includes(n.options.id));
          nodes.forEach(node => {
            const nid = node.options.id;
            node.relationships.associations.forEach((a, ind) => {
              if (node.options.lineColor !== options.lineColor) {
                exe({ type: "onLineColorChanged", data: { id: nid, color: options.lineColor } });
              }
              if (node.options.lineOpacity !== options.lineOpacity) {
                exe({ type: "onLinePropertiesChanged", data: { id: nid, prop: "lineOpacity", val: options.lineOpacity, associationId: a.id, index: ind } });
              }
              if (node.options.lineEffect !== options.lineEffect) {
                exe({ type: "onLinePropertiesChanged", data: { id: nid, prop: "lineEffect", val: options.lineEffect, associationId: a.id, index: ind } });
              }
              if (node.options.lineWidth !== options.lineWidth) {
                exe({ type: "onLinePropertiesChanged", data: { id: nid, prop: "lineWidth", val: options.lineWidth, associationId: a.id, index: ind } });
              }
            });
          });
          break;
        ...
        case "border": {
          invoke({ 
            ...
            compare: ((node) => {
              return node.options.borderColor !== options.borderColor || node.options.borderOpacity !== options.borderOpacity || node.options.borderStyle !== options.borderStyle || node.options.borderWidth !== options.borderWidth; 
            })
          });
          break;
        }
        case "text": {
          invoke({ 
            ...
            packages: [
              { fontSize: options.fontSize, fontFamily: options.fontFamily, fontColor: options.foregroundColor, textOpacity: options.textOpacity },
            ]
          });
          invoke({ 
            type: "onNodeEffectChanged",
            packages: [
              { filter: { apply: "text", id: options.filters.text } },
            ],
            compare: ((node) => {
        ...
        case "image": {
          invoke({ 
            ...
            packages: [
              { img: options.image, w: options.origImage.w, h: options.origImage.h },
            ],
            compare: ((node) => {
        ...
        case "effect": {
          invoke({ 
            type: "onNodeEffectChanged",
            packages: [
              { filter: { apply: "vect", id: options.filters.vect } },
            ],
            compare: ((node) => {
  ...
  return (
    <Dialog maxWidth="md" 
      ...
      <DialogContent style={{minHeight: "300px"}}>
        <DialogTitle id="custom-node-behavior" onClose={closeApplyNodeSettingsDialog}>
          Apply Node Settings
          <Typography variant="body2">
            Select which style(s) to globally apply to all the other nodes on this slate.
          </Typography>
        </DialogTitle>
        <Grid container justify="flex-end" alignItems="center">
          {possibleStyles.map((n, i) => (
            <Grid item key={i} xs={6}>
              <List>
                <ListItem dense role={undefined} button onClick={(e) => { addStyle(n.style); }}>
                  <ListItemIcon>
                    <Checkbox
                      edge="start"
