...
export const NodeShape = (props) => {
  ...
  const updateOptions = async (opts) => {
    ...
    let vbbox = opts.width ? { width: opts.width, height: opts.height } : Slatebox.utils.getBBox({ path: opts.vectorPath }); //size of actual vector
    //console.log("width height before", props.node.options.width, props.node.options.height, vbbox.width, vbbox.height);
    //let path = Slatebox.utils._transformPath(opts.vectorPath, `s${props.node.options.width/vbbox.width},${props.node.options.height/vbbox.height}`);
    let rootWidth = props.node.options.hasResized ? props.node.options.width : 175;
    let rootHeight = props.node.options.hasResized ? props.node.options.height : 100;
    ...
    let path = await promisify(Meteor.call, CONSTANTS.methods.slates.scale, { path: opts.vectorPath, width: ratio, height: ratio });
    ...
    if (props?.node?.options?.opacity === 0) {
      props.onChange({ type: "onNodeColorChanged", data: { opacity: 1, color: props.node.options.backgroundColor } });
    }

    if (opts.isLine) {
      props.onChange({ type: "onNodeBorderPropertiesChanged", data: { id: props?.node?.options?.id, prop: "borderWidth", val: 3 } });
    }
  ...
  const searchNounProject = (e) => {
    async function getResults() {
      shapePage++;
      let results = await promisify(Meteor.call, CONSTANTS.methods.nounProject.get, { limit: searchLimit, query: searchShapeVal, page: shapePage });
      let allshapeItems = shapeItems;
      if (results.data.length > 0) {
        const scaledPaths = results.data.map((d) => {
          return { path: d.path, title: d.title, width: d.width, height: d.height };
        });
  ...
  return (
    <Grid container alignItems="flex-start" justify="center" spacing={1}>
      <Grid item xs={4}>
        <Grid container alignItems="flex-start" justify="flex-start" spacing={1} style={{ maxHeight: "175px", overflow: "scroll" }}>
          {defaultShapes.map((s) => (
            <Grid item key={`${s.key}`}>
              <IconButton aria-label={s.title} style={slateBgStyle} className={classes.shapeItemselect} onClick={() => updateOptions({ vectorPath: s.path, width: s.width, height: s.height, isLine: s.key === "line" })}>
                <SvgIcon htmlColor={bgColor} style={{ fontSize: 33 }} className={classes.shape}>
                  <path strokeWidth={(s.path === path) ? "2px" : (s.key === "line" ? "1px" : "")} stroke={(s.path === path || s.key === "line") ? "#000" :  "none"} d={s.path} />
                </SvgIcon>
              </IconButton>
            </Grid>
          ))}
      ...
      <Grid item xs={6}>
        <InfiniteScroll
          ...
          endMessage={
            (searchShapeVal !== '' && (shapePage > 0 || shapeItems.length > 0)) ?
              ...
              :
              <Grid container alignItems="center" justify="flex-start" style={{ height: "180px" }}>
          ...
          <Grid container alignItems="flex-start" justify="flex-start" spacing={3}>
            {shapeItems && shapeItems.map((s, i) => (
              <Grid item key={`${s.title}_${i}`}>
                <IconButton aria-label={s.title} style={slateBgStyle} className={classes.shapeItemselect}  onClick={() => updateOptions({ vectorPath: s.path, width: s.width, height: s.height })}>
                  <SvgIcon htmlColor={bgColor} style={{ fontSize: 100 }} viewBox="0 0 100 100" className={`${classes.iconHover} ${classes.shape}`}>
