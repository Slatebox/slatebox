...
export const Profile = () => {
  ...
  const [email, setEmail] = Meteor.user() && Meteor.user().emails && Meteor.user().emails[0].address ? React.useState(Meteor.user().emails[0].address) : React.useState(null);
  ...
  async function handleNewEmail() {
    if (email.indexOf("@") > -1) {
      if (email === Meteor.user().emails[0].address) {
        dispatch({ type: "canvas", globalMessage: { visible: true, text: `Your email didn't change! It's still ${email}.`, severity: "error", autoHide: 60000 } });
        return;
      }
      ...
      if (result) {
        await promisify(Meteor.call, CONSTANTS.methods.users.changeEmail, { email });
        dispatch({ type: "canvas", globalMessage: { visible: true, text: `Email sent to ${email}. Please verify.`, severity: "info", autoHide: 60000 } });
      }
    } else {
      dispatch({ type: "canvas", globalMessage: { visible: true, text: `Your email doesn't look right: (${email})`, severity: "error", autoHide: 60000 } });
    }
  }

  function handlePasswordChange() {
    if (newPassword1 !== newPassword2) {
      dispatch({ type: "canvas", globalMessage: { visible: true, text: `Your passwords do not match!`, severity: "error", autoHide: 60000 } });
      return;
    }
    Accounts.changePassword(currentPassword, newPassword1, (err, res) => {
      if (err) {
        console.error(err);
        dispatch({ type: "canvas", globalMessage: { visible: true, text: `Sorry, there was a problem changing your password: ${err.message}`, severity: "error", autoHide: 60000 } });
      } else {
        dispatch({ type: "canvas", globalMessage: { visible: true, text: `Your password has been changed!`, severity: "info", autoHide: 60000 } });
        setCurrentPassword("");
        setNewPassword1("");
        setNewPassword2("");
      }
  ...
  async function handleAccountDeletion() {
    if (!Meteor.user().orgId) {
      ...
      let res = await confirmService.show({
        ...
        actionItems: [{ label: "Yes, remove my account and delete all my slates.", return: true }, { label: "Cancel", return: false }]
      });
      if (res) {
        setTimeout(() => {
        ...
        <Grid item xs={12}>
          {Meteor.user() && Meteor.user().isAnonymous ?
            <Box p={5}>
              ...
              <Box mt={5}>
                <Button color="secondary" size="lg" variant="outlined" onClick={openRegistration}>
                  Register Now
                </Button>
              </Box>
            </Box>
          ...
          <>
                ...
                InputProps={{
                  endAdornment: (
                    <InputAdornment position="end">
                      <Button color="secondary" variant="outlined" onClick={handleNewEmail}>
                        Update
                      </Button>
                    </InputAdornment>
                  ),
            ...
            <Grid item xs={6}>
              <Grid container alignItems="flex-start" justify="flex-end" spacing={2}>
                ...
                <Grid item xs={12}>
                  <Tooltip title="Please enter your current password" placement="top">
                    <TextField
                      variant="outlined"
                      required
                      fullWidth
                ...
                <Grid item xs={12}>
                  <Tooltip title="Type the same password again, at least 8 characters long" placement="top">
                    <TextField
                      variant="outlined"
                      required
                      fullWidth
                ...
                <Grid item>
                  <Button color="secondary" variant="outlined" onClick={handlePasswordChange}>
                    Change Password
                  </Button>
                </Grid>
              </Grid>
