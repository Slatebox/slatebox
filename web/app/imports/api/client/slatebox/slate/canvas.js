...
export default class canvas {
  ...
  init() {
    ...
    self.Canvas = {
      ...
      initDrag: function(e) {
        ...
        if (slate.options.allowDrag) {
          ...
          self.Canvas.objInitialMousePos = { x: m.x + offsets.left, y: m.y + offsets.top };
          var xy = self.cp(e);
    ...
    self.internal.addEventListener('mousedown', (e) => {
      self.slate && self.slate.events && self.slate.events.onCanvasClicked && self.slate.events.onCanvasClicked();
    });
    ...
    if (!slate.options.isbirdsEye && (slate.options.isSharing || slate.options.isEmbedding)) {
      ...
      const _styles = utils.buildStyle({ height: !!_iframe ? (_scaleSize + 8) + "px" : _scaleSize + "px" });
      ...
      if (!slate.options.isbirdsEye && slate.options.isSharing) {
        ...
        ["download", "embed"].forEach((e) => {
          ...
          const _bstyles = utils.buildStyle({ width: _btnSize + "px", height: _btnSize + "px" });
          ...
          utils.addEvent(_btn, "click", function(e) {
            ...
            switch (_act) {
              case "embed":
                ...
                _note.setAttribute("style", utils.buildStyle({ "font-size": "11pt", "text-align": "center", padding: "4px", "margin-right": "-1px", width: "125px", height: "22px", position: "absolute", right: 0, bottom: 0, "background-color": "#333", color: "#fff" }));
  ...
  cp(e) {
    ...
    var difX = this.Canvas.objInitPos.left + (m.x - this.Canvas.objInitialMousePos.x);
    var difY = this.Canvas.objInitPos.top + (m.y - this.Canvas.objInitialMousePos.y);
  ...
  endDrag(coords) {
    ...
    const moved = { x: this.Canvas.objInitPos.left - curPos.left, y: this.Canvas.objInitPos.top - curPos.top };
  ...
  broadcast(moved) {
    this.slate.collab && this.slate.collab.send({ type: "onCanvasMove", data: { left: this.slate.options.viewPort.left, top: this.slate.options.viewPort.top, relative: moved, orient: this.slate.getOrientation() } });
  };
  ...
  zoom(_opts) {
    ...
    var _targetZoom = self.slate.options.viewPort.originalWidth * (100 / parseInt(opts.zoomPercent));
    ...
    emile(self.internal, "padding:1px", {
      ...
      after: function() {
        ...
        opts.callbacks.after && opts.callbacks.after.apply(self.slate, [_targetZoom]);
      },
      during: function(pc) {
    ...
    // } else {
      ...
      if (opts.dur > 0) {
        emile(self.internal, "left:" + (x * -1) + "px;top:" + y * -1 + "px", {
          ...
          after: function() {
            self.slate.options.allowDrag = true;
            self.slate.options.viewPort.left = Math.abs(parseInt(self.internal.style.left.replace("px", "")));
            self.slate.options.viewPort.top = Math.abs(parseInt(self.internal.style.top.replace("px", "")));
            self.slate.birdsEye && self.slate.birdsEye.refresh(true);
  ...
  resize(val) {
    ...
    if (-_top > threshold || -_left > threshold) {
      ...
      this.slate.options.viewPort.zoom = { w: val, h: val, l: parseFloat(_left * -1), t: parseFloat(_top * -1), r: this.slate.options.viewPort.originalWidth / val };
      return true;
    }
    //console.log(val);
  ...
  wire() {
    ...
    for (var ee in this.eve.init) { this.internal[this.eve.init[parseInt(ee)]] = this.Canvas.initDrag; }
    for (var ee in this.eve.drag) { this.internal[this.eve.drag[parseInt(ee)]] = this.Canvas.onDrag; }
    for (var ee in this.eve.up) { this.internal[this.eve.up[parseInt(ee)]] = this.Canvas.endDrag; }
  ...
  unwire() {
    ...
    for (var ee in this.eve.drag) { this.internal[this.eve.drag[parseInt(ee)]] = null; }
    for (var ee in this.eve.up) { this.internal[this.eve.up[parseInt(ee)]] = null; }
    for (var ee in this.eve.gest) { this.internal[this.eve.gest[parseInt(ee)]] = null; }
  };
  ...
  rawSVG(cb) {
    ...
    function extractImages(__svg) {
      let images = uniq(self.slate.nodes.allNodes.map(n => n.options.image).filter(f => !!f));
      if (images.length > 0) {
        images.forEach((i, ind) => {
          if (self.slate.events.onBase64ImageRequested) {
            ...
            utils.toDataUrl(i).then(dataUrl => {
    ...
    embedGoogleFonts({
      ...
      text: uniq(self.slate.nodes.allNodes.flatMap(n => n.options.text.replace(/ /gi, "").split(""))).join("").trim(),
  ...
  refreshBackground() {
    ...
    if (self.slate.options.containerStyle.backgroundEffect) {
      self.slate.options.containerStyle.backgroundColor = self.slate.filters.availableFilters[self.slate.options.containerStyle.backgroundEffect].backgroundColor;
    }
    ...
    if (self.slate.options.containerStyle.backgroundImage) {
      ...
      if (self.slate.options.containerStyle.backgroundSize) {
        self.internal.parentElement.style.backgroundSize = self.slate.options.containerStyle.backgroundSize;
      }
    }
