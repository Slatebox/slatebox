...
export default class Connectors {
  constructor(slate, node) {
    ...
    this.buttons = null
    this.iconBar = null
    this.lastUnpinned = { options: { xPos: null, width: null, yPos: null } }
  }

  broadcast(snap) {
    const pkg = {
      type: 'onNodeAdded',
      data: this.slate.exportDifference(snap),
    }
  ...
  remove() {
    ...
    if (self.iconBar) self.iconBar.remove()
    self.node.menu.isOpen = false
  }
  ...
  createNode(skipCenter, options, targetXPos, targetYPos, snap) {
    ...
    self.lastUnpinned = newNode.options
  ...
  addNode(skipCenter) {
    ...
    const targetXPos =
      (self.lastUnpinned.xPos || self.node.options.xPos) +
        (self.lastUnpinned.width || self.node.options.width || 220) +
        self.node.options.spaceBetweenNodesWhenAdding || 30
    const targetYPos = self.lastUnpinned.yPos || self.node.options.yPos
    ...
    if (!self.slate.options.basedOnThemeId) {
      return self.createNode(skipCenter, options, targetXPos, targetYPos, snap)
    }
