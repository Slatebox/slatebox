...
export default class rotate {
  ...
  getRotationAngle(origPoint, newPoint, centerPoint) {
    ...
    const magnitudeV = Math.sqrt(Math.pow(vx, 2) + Math.pow(vy, 2));
    const magnitudeU = Math.sqrt(Math.pow(ux, 2) + Math.pow(uy, 2));
    let angleDegrees;
  ...
  rta() {
    ...
    let transformString = self.node.getTransformString(rotationContext);
    self.node.vect.transform(transformString);
    self.node.text.transform(transformString);

    // let bb = self.node.vect.getBBox();

    //let rdx = (s.ox - self.node.options.xPos) + dx;
    //let rdy = (s.oy - self.node.options.yPos) + dy;
    // let rdx = dx;
    // let rdy = dy;

    // console.log("rdx rdy ", rdx, rdy);

    // self.slate.nodes.translateRelationships(relationshipsToTranslate, { dx: rdx, dy: rdy });
    // refreshRelationships({relationships: relationshipsToRefresh, nodes: selectedNodes, dx: rdx, dy: rdy });

    // console.log("rotating with relationships ", rotationAngle);
    ...
    //console.log("refreshing relationships ", selectedNodes, relationshipsToRefresh, dx, dy);
    //self.slate.nodes.translateRelationships(relationshipsToTranslate, {dx, dy});
    //refreshRelationships({relationships: relationshipsToRefresh, nodes: selectedNodes });
  ...
  constructor(slate, node) {
    ...
    self.rotateEvents = {
      start: function () {
        ...
        self._initPosFix = false;
        // console.log("lastTextoffset rotate ", this.node.lastTextOffset);
        // this.node.lastTextOffset = null;
        //this.node.text.attr(this.node.textCoords());
        //this.node.relationships.hideOwn();
        self.rotateTemp.attr({ x: self.rotateTemp.attr("x") - 1000, y: self.rotateTemp.attr("y") - 1000, width: 10000, height: 10000 });
        ...
        // const noRotationBB = this.node.vect.getBBox();

        // if (_.isEmpty(_self._.options.rotate.point)) {
        //   _self._.options.rotate.point = {x: noRotationBB.cx, y: noRotationBB.cy};
        // }

        if (!self.node.options.rotate?.point || Object.entries(self.node.options.rotate.point).length === 0) {
          self.node.options.rotate.point = { x: noRotationBB.cx, y: noRotationBB.cy };
        }
        tempPath.remove();
        ...
        // this.node.menu.hide(this.node.options.id);
      ...
      move: function (dx, dy) {
        ...
        try {
          ...
          */
          ...
          self.rotationAngle = self.getRotationAngle({ x: s.ox, y: s.oy }, { x: s.ox + dx, y: s.oy + dy }, self.origCenter);
  ...
  hide(r) {
    this.rotate && this.rotate.remove();
    this.rotateTemp && this.rotateTemp.remove();
  };

  send() {
    //broadcast change to birdsEye and collaborators
    var pkg = {
      type: 'onNodeRotated', data: {
  ...
  applyImageRotation(opts = {}) {
    const r = this.slate.options.viewPort.zoom.r;
  ...
  animateSet(data, opts) {
    ...
    self.node.vect.animate({ transform: transformString }, opts.duration || 500, opts.easing || ">", function () {
    ...
    self.node.text.animate({ transform: transformString }, opts.duration || 500, opts.easing || ">", function () {
      const tstr = self.node.getTransformString();
      //self.node.vect.transform(tstr);
      self.node.text.transform(tstr);
      //opts.cb && opts.cb();
