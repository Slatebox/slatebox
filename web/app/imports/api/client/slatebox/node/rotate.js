...
export default class Rotate {
  ...
  finalize() {
    const self = this
    const ran = self.node.snappedAt || self.rotationAngle
    if (ran) {
      const rotationContext = {
        rotationAngle: (self.node.options.rotate.rotationAngle + ran) % 360,
        point: self.origCenter,
      }
    ...
    if (self.node.events?.onRotated?.apply) {
      self.node.events?.onRotated?.apply(self, [self.send])
    } else {
      self.send()
    }
  }
  ...
  show(x, y) {
    ...
    if (self.node.options.allowResize) {
      if (self.rotateTemp) self.rotateTemp.remove()
      ...
      const rotatePathString = Utils.lowLevelTransformPath(
      ...
      self.rotateTemp = r
        ...
        .toFront()
      self.rotate.mouseover(() => {
        self.rotate.attr({ cursor: 'alias' })
      })

      self.rotateTemp.mouseover(() => {
  ...
  hide() {
    if (this.rotate) this.rotate.remove()
    if (this.rotateTemp) this.rotateTemp.remove()
  }
