...
export default class gridLines {
  ...
  clear() {
    const self = this;
    ["rightVerticals", "leftVerticals", "topHorizontals", "bottomHorizontals"].forEach(p => {
      if (self.guideLines[p]) {
        Object.keys(self.guideLines[p]).forEach(k => self.guideLines[p][k].remove());
      }
      self.guideLines[p] = {};
    });
  }
  ...
  draw(id, dx, dy, bb, translateNode = true, guideVisibleAt = 100, guideSnapAt = 20) {
    ...
    const self = this;
    let snapToObjects = self.slate.options.viewPort.snapToObjects != null ? self.slate.options.viewPort.snapToObjects : true;
    let pinnedY = false;
    let pinnedX = false;
    const nbb = self.node.vect.getBBox();
    self.guideAttrs.stroke = utils.whiteOrBlack(self.slate.options.containerStyle.backgroundColor);
    ...
    if ((nbb.x - bb.x2 < guideVisibleAt && nbb.x - bb.x2 >= 0) || (bb.x2 - nbb.x2 < guideVisibleAt && bb.x2 - nbb.x2 >= 0)) {
      if (!self.guideLines.rightVerticals[id]) {
        self.guideLines.rightVerticals[id] = self.slate.paper.path(`M ${bb.x2} ${bb.y - self.guideWings} L ${bb.x2} ${bb.y2 + self.guideWings}`).attr(self.guideAttrs);
      }
      ...
      self.guideLines?.rightVerticals[id] && self.guideLines?.rightVerticals[id].remove();
    ...
    if (!pinnedX && ((bb.x - nbb.x2 < guideVisibleAt && bb.x - nbb.x2 >= 0) || (nbb.x - bb.x < guideVisibleAt && nbb.x - bb.x >= 0))) {
      if (!self.guideLines.leftVerticals[id]) {
        self.guideLines.leftVerticals[id] = self.slate.paper.path(`M ${bb.x} ${bb.y - self.guideWings} L ${bb.x} ${bb.y2 + self.guideWings}`).attr(self.guideAttrs);
      }
      ...
      self.guideLines?.leftVerticals[id] && self.guideLines?.leftVerticals[id].remove();
    ...
    if ((bb.y - nbb.y2 < guideVisibleAt && bb.y - nbb.y2 >= 0) || (nbb.y - bb.y < guideVisibleAt && nbb.y - bb.y >= 0)) {
      if (!self.guideLines.topHorizontals[id]) {
        self.guideLines.topHorizontals[id] = self.slate.paper.path(`M ${bb.x - self.guideWings} ${bb.y} L ${bb.x2 + self.guideWings} ${bb.y}`).attr(self.guideAttrs);
      }
      ...
      self.guideLines?.topHorizontals[id] && self.guideLines?.topHorizontals[id]?.remove();
    ...
    if (!pinnedY && ((nbb.y - bb.y2 < guideVisibleAt && nbb.y - bb.y2 >= 0) || (bb.y2 - nbb.y2 < guideVisibleAt && bb.y2 - nbb.y2 >= 0))) {
      if (!self.guideLines.bottomHorizontals[id]) {
        self.guideLines.bottomHorizontals[id] = self.slate.paper.path(`M ${bb.x - self.guideWings} ${bb.y2} L ${bb.x2 + self.guideWings} ${bb.y2}`).attr(self.guideAttrs);
      }
      ...
      self.guideLines?.bottomHorizontals[id] && self.guideLines?.bottomHorizontals[id]?.remove();
