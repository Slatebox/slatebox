...
export default class LineOptions {
  ...
  broadcast(pkg) {
    const upkg = pkg
    upkg.data.id = this.node.options.id
  ...
  show(e, c) {
    ...
    const a = self.node.relationships.associations.find((a) => a.id === c.id)
    // if (!self.m[c.id]) {
    ...
    if (reassign) {
      reassign.mousedown((ex) => {
        self.node.relationships.initiateTempNode(ex, c.parent, {
