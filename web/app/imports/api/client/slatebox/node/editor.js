...
export default class editor {
  ...
  setTextOffset() {
    if (this.node.options.allowDrag) {
      ...
      this.node.options.textOffset = { x: (this.node.options.textBounds.cx - this.node.options.textBounds.width / 2) - this.node.options.xPos, y: this.node.options.textBounds.cy - this.node.options.yPos, width: this.node.options.textBounds.width, height: this.node.options.textBounds.height };
    }
  };

  set(t, s, f, c, opacity, ta, tb, useXY) {

    let tempShim = `§`; //utils.guid().substring(3);
    ...
    coords = this.node.textCoords({ x: this.node.options.xPos, y: this.node.options.yPos });
    ...
    const noSelect = ["-webkit-user-select", "-moz-user-select", "-ms-user-select", "user-select"].map(s => `${s}: none;`).join(" ");
