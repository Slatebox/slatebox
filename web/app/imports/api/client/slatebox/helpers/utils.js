...
export default class utils {
  ...
  static availColors = [
    ...
    , {
      special: { //line options display only colors; node menu displays colors and transparent button
  ...
  static select(query) {
    ...
    if (index != -1) {
      let tag = query.slice(0, index) || "*";
      let klass = query.slice(index + 1, query.length);
      let els = [];
      let all = document.getElementsByTagName(tag);
  ...
  static toShortDateString(jsonDate) {
    let _date = jsonDate;
    try {
      let d = new Date(parseInt(jsonDate.substr(6)));
  ...
  static imageExists(u, cb, id) {
    let _id = "temp_" + guid();
    let _img = document.body.appendChild(document.createElement("img"));
    ...
    addEvent(_img, "load", function (e) {
      let d = getDimensions(_img);
      document.body.removeChild(_img);
      cb.apply(this, [true, d.width, d.height, id]);
    });

    addEvent(_img, "error", function (e) {
      document.body.removeChild(_img);
  ...
  static ajax(u, f, d, v, x, h) {
    ...
    let vx = d ? (v ? v : 'POST') : (v ? v : 'GET');
    x.open(vx, u, 1);
    x.setRequestHeader('Content-type', 'application/json; charset=utf-8');
    h.forEach(function (hElem) { x.setRequestHeader(hElem.n, hElem.v); });
  ...
  static S4() { return (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1); }
  static guid() { return (this.S4() + this.S4() + this.S4()); }
  static number() { return Math.floor(Math.random() * 9999) + 999; }

  static getJSON(url, callback) {
    let id = S4() + S4();
    let script = document.createElement('script'), token = '__jsonp' + id;
  ...
  static onOff(baseUrl, ele, callback) {
    let imgID = guid().replace('-', '').substring(0, 8);
    let _element = ensureEle(ele);
  ...
  static div(p, x, y, w, h) {
    let _id = "temp_" + guid();
    let _div = p.appendChild(document.createElement("div"));
  ...
  static centerAndScalePathToFitContainer(opts) {
    ...
    let _newPath = this._transformPath(opts.path, ["s", _scale, ",", _scale].join(""));
    ...
    //of the enclosed box
    const tp = ["T", ((bb.x * -1) + ((opts.containerSize - bb.width) / 2)), ",", (bb.y * -1 + ((opts.containerSize - bb.height) / 2))].join("");

    _newPath = this._transformPath(_newPath, tp); //s" + _scale + "," + _scale).toString();
  ...
  static whiteOrBlack(hex) {
    return Raphael.rgb2hsb(hex).b < .4 ? "#fff" : "#000";
  };
  ...
  static transformPath(_node, _transformation) {
    const _path = Raphael.transformPath(_node.vect.attr("path").toString(), _transformation).toString();
    _node.options.vectorPath = _path;
    _node.vect.transform("");
    _node.vect.attr({ path: _node.options.vectorPath });
    let bb = _node.vect.getBBox();
    let rotationContext = {
      point: {
        x: bb.cx,
        y: bb.cy
  ...
  static toDataUrl = url => fetch(url, { mode: "cors" })
