...
export default function getCorrectMidPoints(opts = {}) {
  ...
  const pcx = isNumber(originBB.cx) && originBB.cx
  const pcy = isNumber(originBB.cy) && originBB.cy
  const ccx = isNumber(endBB.cx) && endBB.cx
  const ccy = isNumber(endBB.cy) && endBB.cy
