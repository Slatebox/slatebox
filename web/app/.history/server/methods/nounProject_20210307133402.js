...
method[CONSTANTS.methods.nounProject.get] = async function(opts) {
  ...
  const NO_CACHE = true;
  const PAID_NOUNPROJECT_ACCOUNT = false;
  ...
  if (Meteor.user()) {
    ...
    if (_priorAttempt || _currentPageExceedsData.length > 0) { // || _emptyFutureAttempts.length > 0
      ...
      function _getPaths(cb) {
        ...
        if (PAID_NOUNPROJECT_ACCOUNT) {
          delete sigParams.limit_to_public_domain;
        }
              ...
              if (err) {
                ...
                try {
                  ...
                  if (initPath.length > 0) {
                    ...
                    r.optimizedPath = `${consolidatePaths(reoptimize.data).replace(/z/gi, " ")}z`; //scaledPath;
                    console.log("final optimized path ", r.optimizedPath.length);
                    _all.push({ title: r.term, path: r.optimizedPath, width: width, height: height });
                    console.log("optimized complete ", r.optimizedPath);
