...
import { Organizations } from '/imports/api/common/models.js';
...
export default setup = function() {
  ...
  function _compileEmail(user, url, type, asText) {
    ...
    let _header = `<div style="width:800px;margin-right:auto;margin-left:auto;">
    ...
    switch (type) {
      case "verify":
        if (asText) {
          _body = `Yay for joining Slatebox! Please verify your email below.\n\n
              Click here to verify your email: ${url}\n
              Once verified, you'll be able to access all the Slatebox features.\n
              We're excited to see what you create!`;
        } else {
          _body = `<div style="padding:3px;margin:3px;font-family:Helvetica;font-size:13pt;">
      ...
      // case "enrollAdmin":
      //   if (asText) {
      //     _body = `Your new Slatebox ${name} team is ready!\n\n
      ...
      case "enrollInvite":
        if (asText) {
          _body = `Hooray! You've been invited to join the ${name} team.\n\n
      ...
      case "forgot":
        if (asText) {
          _body = `Hi ${userName}, forget your password? No problem.\n\n
    ...
    const _ret = `${_header}${_body}${footer}`;

    console.log("compiled email ", _ret);
