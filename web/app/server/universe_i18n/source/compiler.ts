...
class UniverseI18nCompiler extends CachingCompiler {
  ...
  compileOneFile(file: InputFile): CompileResult | null {
    ...
    if (!locale) {
      ...
      console.log("metadata is ", metadata, options);
      console.log("file is ", metadata.locale, options.locale, file);
      file.error({ message: `Cannot find locale for file22 "${file}"${extra}.` });
      return null;
    }
