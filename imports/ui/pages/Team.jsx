...
export const Team = (props) => { 
  ...
  let verb = AuthManager.userHasClaim(uid, [CONSTANTS.claims.canEditUsers._id, CONSTANTS.claims.canAddUsers._id]) ? "Manage" : "View";
  let abilities = [];
  if (AuthManager.userHasClaim(uid, CONSTANTS.claims.admin._id)) {
    abilities = abilities.concat(["add new ones", "edit their info and change their roles", "remove them"]);
  } else {
  ...
  return (
    <Container component="main" maxWidth="lg">
      <Grid container spacing={4} alignItems="center" justify="flex-end">
        ...
        <Grid item xs={12}>
         { (!org && Meteor.user() && (Meteor.user().isAnonymous || !Meteor.user().emails[0].verified)) ? 
            <>
              <Typography variant="h5" color="secondary">You don&apos;t have a team yet. Create one now!</Typography>
              <Box m={3}>
                <Button size="large" variant="outlined" onClick={createTeam}>Create your team!</Button>
              </Box>
            </>
         :
          <>
            ...
            <Typography variant="subtitle1" color="textPrimary">
              {abilities.slice(0, abilities.length-1).join(", ")} {abilities.slice(abilities.length-1, abilities.length).join("")}
            </Typography> 
          </>
          }
        </Grid>
