...
export const SnapSlate = (props) => {
  ...
  useEffect(() => {
    ...
    if (ss) {
      ...
      const snap = new Slatebox.slate({
        container: props.overrideContainer ? `slatePreviewContainer` : `slate_${ss.shareId}`
        , containerStyle: { backgroundColor: ss.options.containerStyle.backgroundColor }
        ...
        , collaboration: {
          ...
          onCollaboration: async function (opts) {
            ...
            console.log("onCollaboration snap called", opts.type, opts.pkg?.type);
          }
        }
      }, events).init();
        ...
        Collaboration.find({ slateId: ss.shareId }).observe({
          added: function (doc) {
            ...
            if (doc.type === "onCanvasMove") {
              ...
              const ratio = Math.max(orient.width, orient.height) / Math.max(doc.data.orient.width, doc.data.orient.height);
  ...
  return (<Box id={props.overrideContainer ? `slatePreviewContainer` : `slate_${props.slate?.shareId}`} className={props.allowDrag ? classes.noZoom : classes.slateTheme}></Box>);
}