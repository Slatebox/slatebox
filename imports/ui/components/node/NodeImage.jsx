...
export const NodeImage = (props) => {
  ...
  const imgDominantColor = useSelector(state => state.imgDominantColor) || "all";
  ...
  const handleSearch = (e) => {
    ...
    async function search() {
      ...
      if (imgDominantColor && imgDominantColor !== "all") filter.push(`&imgDominantColor=${imgDominantColor}`);
      if (imgColorType && imgColorType !== "all") filter.push(`&imgColorType=${imgColorType}`);
      const imgFilter = filter.join("");
      let url = `https://customsearch.googleapis.com/customsearch/v1?cx=${Meteor.settings.public.googleImageSearchEngineKey}&num=${limit}&safe=ACTIVE&searchType=image&key=${Meteor.settings.public.googleImageSearchAPIKey}&start=${p * limit}&q=${imageSearchVal}${imgFilter}`;
      ...
      if (sessionStorage.getItem(`sb_${url}`)) {
        ...
        let next = parseInt(sessionStorage.getItem(`sb_${imageSearchVal}`), 10) + 1;
        ...
        dispatch({
          ...
          , imagePage: p
          , isLoading: false
          , hasMoreImages: d?.queries?.nextPage?.length > 0 && parseInt(sessionStorage.getItem(`sb_${imageSearchVal}`), 10) < 3
        });
      } else {
    ...
    <Grid container alignItems="flex-start" justify="space-between" spacing={4}>
      <Grid item xs={2}>
        ...
        <FormControl variant="outlined" className={classes.formControl}>
          ...
          >
            <MenuItem value="all"><Translation>nodeDrawer.images.imgColorType.all</Translation></MenuItem>
            <MenuItem value="color"><Translation>nodeDrawer.images.imgColorType.color</Translation></MenuItem>
            <MenuItem value="gray"><Translation>nodeDrawer.images.imgColorType.gray</Translation></MenuItem>
            <MenuItem value="mono"><Translation>nodeDrawer.images.imgColorType.blackAndWhite</Translation></MenuItem>
            <MenuItem value="trans"><Translation>nodeDrawer.images.imgColorType.transparentBackground</Translation></MenuItem>
          </Select>
        </FormControl>
      ...
      <Grid item xs={2}>
        ...
        <FormControl variant="outlined" className={classes.formControl}>
          ...
          <Select
            ...
            label="Image Size"
          >
            <MenuItem value="all"><Translation>nodeDrawer.images.imgSize.all</Translation></MenuItem>
            <MenuItem value="icon"><Translation>nodeDrawer.images.imgSize.icon</Translation></MenuItem>
            <MenuItem value="small"><Translation>nodeDrawer.images.imgSize.small</Translation></MenuItem>
            <MenuItem value="medium"><Translation>nodeDrawer.images.imgSize.medium</Translation></MenuItem>
            <MenuItem value="large"><Translation>nodeDrawer.images.imgSize.large</Translation></MenuItem>
            <MenuItem value="xlarge"><Translation>nodeDrawer.images.imgSize.xlarge</Translation></MenuItem>
            <MenuItem value="xxlarge"><Translation>nodeDrawer.images.imgSize.xxlarge</Translation></MenuItem>
            <MenuItem value="huge"><Translation>nodeDrawer.images.imgSize.huge</Translation></MenuItem>
          </Select>
        </FormControl>
        ...
        <FormControl variant="outlined" className={classes.formControl}>
          ...
          <Select
            ...
            label="Image Type"
          >
            <MenuItem value="all"><Translation>nodeDrawer.images.imgType.all</Translation></MenuItem>
            <MenuItem value="clipart"><Translation>nodeDrawer.images.imgType.clipart</Translation></MenuItem>
            <MenuItem value="face"><Translation>nodeDrawer.images.imgType.face</Translation></MenuItem>
            <MenuItem value="lineart"><Translation>nodeDrawer.images.imgType.lineart</Translation></MenuItem>
            <MenuItem value="stock"><Translation>nodeDrawer.images.imgType.stock</Translation></MenuItem>
            <MenuItem value="photo"><Translation>nodeDrawer.images.imgType.photo</Translation></MenuItem>
            <MenuItem value="animated"><Translation>nodeDrawer.images.imgType.animated</Translation></MenuItem>
          </Select>
        </FormControl>
      </Grid>
      <Grid item xs={2}>
        <Grid container alignItems="center" justify="flex-end" style={{ height: "140px" }}>
          <TextField
        ...
        <InfiniteScroll
          ...
          endMessage={
            (imageSearchVal !== '' && imagePage > 0) ?
              ...
              :
              <Grid container alignItems="center" justify="flex-start" style={{ height: "140px" }}>
                <Typography variant="body2" style={{ color: "#fff" }}>
        ...
        >
          <GridList cols={3}>
            {imageItems.map((image, i) => (
              <GridListTile className={classes.imageItemselect} key={i} cols={1} aria-label={image.title} onClick={() => updateOptions({ image: image.link, width: image.image.width, height: image.image.height, selectedImage: image.image.thumnailLink })}>
                <img
                  style={(image?.image?.thumbnailLink === selectedImage) ? { border: "1px solid black" } : {}}
                  src={["huge", "xxlarge", "xlarge", "large"].includes(imgSize) ? image?.image?.thumbnailLink : image.link}
                  alt={image.image.title}
                  onError={this.src = 'data:image/jpeg;base64,SU1BR0UgTk9UIEZPVU5E'}
                />
