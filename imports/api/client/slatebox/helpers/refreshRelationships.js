...
export default function refreshRelationships({relationships, nodes, dx = 0, dy = 0}) {
  relationships.forEach((r) => {
    const midPoints = getCorrectMidPoints(r);

    let tempOriginNode, tempEndNode, linePath;
    if (nodes.some((n) => { return n.options.id === r.parent.options.id; })) {
      ...
      tempEndNode = r.child.getTempPathWithCorrectPositionFor({ //this one is all about rotation
        pathElement: r.child.vect,
        dx: 0,
        dy: 0
      });
      const childPathContext = closestPoint(tempEndNode || r.child.vect, midPoints.parent);
      const pointOnChildPath = childPathContext.bestPoint;

      const parentPathContext = closestPoint(tempOriginNode || r.parent.vect, pointOnChildPath);
      ...
      tempOriginNode = r.parent.getTempPathWithCorrectPositionFor({ //this one is all about rotation
        pathElement: r.parent.vect,
        dx: 0,
        dy: 0
      });
    ...
    let _attr = { stroke: r.lineColor, fill: "none", "stroke-width": r.lineWidth, "fill-opacity": r.lineOpacity, opacity: r.lineOpacity, filter: r.lineEffect ? `url(#${r.lineEffect})` : "" };
