...
import { SVGPathDataTransformer, SVGPathDataParser } from 'svg-pathdata';
...
export const NodeShape = (props) => {
  ...
  const updateOptions = (opts) => {
    ...
    let vbbox = Slatebox.utils.getBBox({ path: opts.vectorPath }); //size of actual vector
    //console.log("width height before", props.node.options.width, props.node.options.height, vbbox);

    const transformingParser = new SVGPathDataParser().toAbs().scale(props.node.options.width/vbbox.width, props.node.options.height/vbbox.height);
    const updatedPath = transformingParser.parse(opts.vectorPath) // returns [{ type: SVGPathData.MOVE_TO,       relative: false, x: 0, y: 0 }]
